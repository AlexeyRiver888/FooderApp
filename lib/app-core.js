...
export function scheduledStatusForNow(date = new Date()) {
  ...
  const joinStart = 11 * 60;
  const voteStart = 11 * 60 + 20;
  if (now.minuteOfDay < joinStart) return "waiting";
  if (now.minuteOfDay < voteStart) return "joining";
  return "voting";
}
...
export function applyScheduleToSession(session, venues, date = new Date()) {
  ...
  if (session.status === "waiting" || session.status === "joining" || session.status === "closed") {
    session.status = session.activeUsers.length >= 3 && venues.length > 0 ? "voting" : "closed";
  }
  return session;
}
