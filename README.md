...
Сейчас приложение автоматически создает инвайт:

```txt
demo
```

Для первого запуска используй его.

Позже можно будет сделать админку или отдельный скрипт для создания новых инвайтов. Пока проще оставить `demo`, чтобы проверить весь путь до конца.
