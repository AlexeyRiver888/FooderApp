...
## Как отправить сообщение всем через бота

Если твой Telegram ID указан в `ADMIN_TELEGRAM_IDS`, ты можешь написать своему боту:
...
```txt
/broadcast Сегодня идем пораньше, голосование начнется через 5 минут
```

Бот отправит это сообщение всем пользователям, которые уже открывали mini-app и попали в базу `users`.
