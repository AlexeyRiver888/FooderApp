...
4. Нажми `Погнали`.
5. Когда минимум 3 человека нажали `Погнали`, появится кнопка `Голосовать`.
6. Нажми `Голосовать`.
7. Попробуй свайпы:
   - вправо = да;
   - влево = нет;
   - вверх = ветто.
8. Когда все участники проголосовали по всем заведениям, появится кнопка `Узнать куда идем?`.
9. После раскрытия результата бот отправит участникам сообщение с названием и адресом победителя.
